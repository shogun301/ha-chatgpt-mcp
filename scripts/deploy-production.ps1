...
archive_path='/tmp/ha-chatgpt-mcp-2.7.4.tar.gz'
...
python3 -c 'import json; p=json.load(open("/tmp/ha-mcp-preflight-health.json", encoding="utf-8")); assert p.get("status") == "ok" and p.get("service_version") == "2.7.4"'
...
assert payload.get('service_version') == '2.7.4'
PY
  ...
  case "$ha_public_status" in
    200|302|403)
      if curl --fail --silent --max-time 10 __PUBLIC_MCP_URL__/healthz \
         ...
         python3 -c 'import json; p=json.load(open("/tmp/ha-mcp-public-health.json", encoding="utf-8")); assert p.get("status") == "ok" and p.get("service_version") == "2.7.4"'; then
