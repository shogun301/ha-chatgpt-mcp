...
Assistant capability synchronization added in MCP 2.6.0.
...
an isolated container, installs/enables the included systemd unit, deploys MCP
   2.6.0, and performs read-only health checks. Do not hand-copy secrets or add a
Docker-socket mount.

Before deployment:

1. Confirm the release version is 2.6.0 and the expected tool count is 99.
...
4. **MCP registry:** authenticated discovery reports version 2.6.0 and exactly
...
8. **Capability persistence:** call `get_capability_sync_status` with refresh,
   ...
   version 2.6.0, then confirm the file remains under `/data` across MCP restart.
